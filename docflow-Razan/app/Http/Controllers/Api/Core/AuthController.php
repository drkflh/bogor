...
class AuthController extends Controller {
    ...
    public function postLoginToken(Request $request){
        ...
        $user = Member::where( function ($q) use ($login){
            $q->where('nik', '=', $login)
                ->orWhere('email', '=', $login)
    ...
    public function postReset(Request $request){
        ...
        if($em == 0){
            $retVal = array("result" => "ERR", "message" => "Email tidak ditemukan, silakan register terlebih dahulu.", 'data'=>false);
            return response()->json($retVal);
        }
        ...
        $retVal = ["result" => "OK",
            "message" => "Email berisi URL untuk reset password telah dikirim",
            'data'=>$member['email']
        ];
