...
class Util {
    ...
    public static function log($actor, $url, $method ,$req, $resp, $authItem = '', $itemId = '')
    {
        ...
        $actionlog->actorId = $actor['_id'];
        $actionlog->actorName = $actor['name'];
...
//        Route::get('/'.$controller.'/commit/{sessid}', $controller_class.'@getCommit');
    ...
    public function toTemplateHeader()
    {
        ...
        if( self::$yml ){
            foreach( self::$yml as $k=>$v){
                if( isset($v['name']) && $v['name'] != '' && !(in_array( $v['name'] ,config('util.template_exclude') ) ) ){
        ...
        if( self::$yml ){
            foreach( self::$yml as $k){
                if( isset($k['name']) && ( isset($k['show']) && $k['show'] == true ) ){
                    ...
                    $fields[ $k['name'] ]['label'] = $k['label']??'Action';
            ...
            foreach($menu as $m){
                if(isset($m['auth'])){
                    if($roleEntity->can('read', $m['auth'] , $roleId)){
                        $menuarray[] = sprintf($section, $m['title']);
                        $next = true;
                    }else{
                        $next = false;
                    }
                                        ...
                                        if(isset($sm['children'])  && $next  ){
                                            ...
                                            if(isset( $sm['auth'] ) && !is_null($roleEntity) ){
                                                if($roleEntity->can('read', $sm['auth'] , $roleId)){
                                                    $submenu[] = sprintf($subparent, $icon , $sm['title'] , $gcmenustring, $badge, $badge );
                                                }
                                            }else{
                                                $submenu[] = sprintf($subparent, $icon , $sm['title'] , $gcmenustring, $badge, $badge );
                                            }
                                            ...
                                            if(isset( $sm['auth'] ) && !is_null($roleEntity)){
                                                ...
                                                if($roleEntity->can('read', $sm['auth'] , $roleId)){
                                                    if(isset($sm['title'])){
                                                        $submenu[] = sprintf($subitem, $surl, $sm['title'], $badge, $badge );
                                                    }
                                                }
                                            }else{
                                                if(isset($sm['title'])){
        ...
        foreach ($children as $ch){
            ...
            if(isset( $ch['auth'] ) && !is_null($roleEntity)){
                if($roleEntity->can('read', $ch['auth'] , $roleId)){
                    $gc[] = sprintf($subitem, $surl, $ch['title'], $badge, $badge );
                }
            }else{
                $gc[] = sprintf($subitem, $surl, $ch['title'], $badge, $badge );
            }
    ...
    public function composeFormElement($k, $v, $is_create = true, $model_prefix = '')
    {
        ...
        $field = Former::text($v['name'],$k)->v_model( $model );
    ...
    public function composeFormField($k, $v, $is_create = true)
    {
        ...
        if( $v['form']['type'] == 'text' ){
            $field = Former::text($v['name'],$k)
                ->v_model( $model );
        }

        if( $v['form']['type'] == 'hidden' ){
            $field = Former::hidden($k)
                ->v_model( $model );
        }

        if( $v['form']['type'] == 'textarea' ){
            $class = (isset($v['form']['attr']['class']))?$v['form']['attr']['class']:'form-control';
            $rows = (isset($v['form']['attr']['rows']))?$v['form']['attr']['rows']:15;
            $field = Former::textarea($v['name'],$k)->v_model( $model )->class($class)->rows($rows);
        }

        if( $v['form']['type'] == 'select' ){
            $field = Former::select($v['name'],$k)->v_model( $model )->options($options, $default);
        }

        if( $v['form']['type'] == 'file' ){
            $options = [
                'options'=>$v['form']['options'],
                'onsending'=>$v['form']['onsending'],
                'onuploadcomplete'=>$v['form']['onuploadcomplete'],
                'imagevar'=>$v['form']['imagevar'],
            ];

            $field = view('form.component.file', $options )->render();
        }

        if( $v['form']['type'] == 'password' ){
            $field = Former::password($v['name'],$k)->v_model( $model );
        }
        ...
        $is_former = ( stripos('former', strval(get_parent_class( $field )) ));


        if($is_former){
            if(isset($v['form']['class'])){
                $field->class( 'form-control '.$v['form']['class'] );
            }

            if(isset($v['form']['attr'])){
                $attrs = $v['form']['attr'];
