...
@endif
        <currency-input
            ...
            @if(isset($form['attr']) && is_array($form['attr']))
                ...
                @endforeach
            @endif
            @if(isset($form['precision']))
                :precision="{ min: {{ $form['precision'] }} , max: {{ $form['precision'] }}  }"
            @endif
            :distraction-free="{ hideNegligibleDecimalDigits: {{ $form['distraction_free'] ?? 'true' }} }"
            locale="{{ $form['locale'] ?? 'id' }}"
            :auto-decimal-mode="{{ $form['auto_decimal_mode'] ?? 'false' }}"
        />
@if(env('SKIP_VALIDATION', false) == false)
