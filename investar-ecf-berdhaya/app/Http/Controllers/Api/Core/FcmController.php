...
class FcmController extends Controller {
    ...
    public function postRegister(Request $request)
    {
        ...
        if( $token == $prevToken ){
            ...
            $efcm = Fcm::where('token','=', $token)->first();

            if($efcm ){
                $efcm->token = $token;
                $efcm->prevToken = $prevToken;
                $fcm->currentUserId = $currentUser['extId'] ?? '';
                $fcm->currentUser = $currentUser;
                $efcm->save();
            ...
            $efcm = Fcm::where('prevToken','=', $prevToken)->first();
        ...
        return response()->json(
            [
                'result'=>'OK',
                'message'=>'Token registered or updated',
                'data'=>[
                    'token'=>$token
                ]
            ],
