...
class InvestmentBudgetController extends AdminController

{
    public function __construct()
    {
        ...
        $this->yml_file = 'investmentbudget_controller';

        $this->entity = 'Budget Investasi';

        $this->auth_entity = 'ecf-investment-budget';

        $this->controller_base = 'ecf/investmentbudget';

        $this->view_base = 'ecf.investmentbudget';

        $this->title_fields = 'name';

        $this->model = new InvestmentBudget();
    }

    public function getIndex()
    {
        $this->title = 'Budget Investasi';
        ...
        */
        ...
        $this->form_layout = 'ecf.investmentbudget.form_layout';
        $this->form_dialog_size = 'md';

        /**
        * Set Viewer layout
        */
        $this->viewer_layout = 'ecf.investmentbudget.view_layout';
...
//        $this->defOrderDir = 'asc';
    ...
    public function additionalQuery($model, Request $request)
    {
        ...
        if( $adv && $ext &&  (isset($adv['enable']) && $adv['enable']) && $adv['isOpen']){
            ...
            if( isset($ext['value']) && $ext['value'] != '' ){
                $value = explode(',', $ext['value'] );

                $model = $model->where(function($q) use ( $value) {
                    for($i = 0; $i < count($value); $i++){
                        if($i == 0){
                            $q = $q->where('value','like','%'.$value[$i].'%');
                        }else{
                            $q = $q->orWhere('value','like','%'.$value[$i].'%');
                        }
                    }
                });
            }
