...

{
    public function __construct()
    {
        ...
        $this->view_base = 'sfm.store.payment';
    ...
    public function paymentConfirmation(Request $request){
        ...
        if($result <= 0){
            ...
            $this->model2->fname = $fname;
            $this->model2->billing_address = $billing_address;
            $this->model2->billing_address2 = $billing_address2;
            $this->model2->city = $city;
            $this->model2->zipcode = $zipcode;
            $this->model2->mobile = $mobile;
            $this->model2->email = $email;
            $this->model2->add_info = $add_info;
            $this->model2->delivery_option = $delivery_option;
            $this->model2->shippingCost = $shippingCost;
            $this->model2->save();
        }else{
            $this->model2->where("cartSession","=", $cartSession)
            ->update(
                ["status" => "Payment","email" => $email,"mobile" => $mobile,
                "zipcode" => $zipcode,"city" => $city,
                "fname" => $fname, "lname" => $lname, "billing_address" => $billing_address,
                "billing_address2" => $billing_address2,"shippingCost" => $shippingCost,
                "delivery_option" => $delivery_option,"add_info" => $add_info]
            );
        }
        ...
        $this->form_layout = 'sfm.payment.form_layout';
    ...
    public function proceedConfirmation(Request $request){
        ...
        foreach($cartShop as $i => $cartShop)
        {
               ...
               $salesOrder[$i] = (new SalesOrder())->forceCreate($cartShop->only(['productName', 'productCode', 'price', 'category', 'unit', 'unitCount', 'weight', 'cartSession', 'orderPrice', 'orderSubTotal', 'orderQty', 'orderTime']));
               // Stock to calculate
               $stock = $salesOrder[$i]['orderQty']; 

               // Tambah ke inventory
               $inventory[$i] = (new InventoryItem())->forceCreate($cartShop->only(['productCode', 'cartSession']));       
               $productCode = $inventory[$i]['productCode'];
               
               $product= $this->model5::where('productCode', $productCode)->first();              

               // Add to Inventory Stock with status=UNCONFIRMED , change to CONFIRMED after PAYMENT->CONFIRMED 
               $this->model4->where("cartSession","=", $cartSession)->where('productCode','=',$productCode)
               ->update(
                   ['status' =>  'UNCONFIRMED', 
               ...
               $this->model5->where('productCode','=',$productCode)
               ->update(
                   ['status' =>  'UNCONFIRMED', 
                   'stock' => $new_stock,
                   'lastStock' => $old_stock
        ...
        $this->model3->buyer = $fullname;
        $this->model3->fname = $cartShopAt['fname'];
        $this->model3->lname = $cartShopAt['lname'];
        $this->model3->billing_address = $cartShopAt['billing_address'];;
        $this->model3->billing_address2 = $cartShopAt['billing_address2'];;
        $this->model3->city = $cartShopAt['city'];
        $this->model3->zipcode = $cartShopAt['zipcode'];
        $this->model3->mobile = $cartShopAt['mobile'];
        $this->model3->email = $cartShopAt['email'];
        $this->model3->add_info = $cartShopAt['add_invo'];
        $this->model3->orderDate = Carbon::now();
        $this->model3->delivery_option = $cartShopAt['delivery_option'];
        $this->model3->payment_option = $cartShopAt['payment_option'];
        $this->model3->shippingCost = $cartShopAt['shippingCost'];    
        ...
        $this->model3->orders = $cartShop;
        $this->model3->weight = $weight;
        ...
        //  $this->item_data_url = 'sfm/invoice/data/'.$inv;
        ...
        //  $this->form_layout = 'sfm.payment.invoice_layout';
    ...
    public function getInvoice($id, $additional_data = null)
    {
        ...
        $this->item_data_url = 'sfm/invoice/data/';
    ...
    {        
        ...
        $this->form_layout = 'sfm.payment.invoice_data';
