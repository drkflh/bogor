        ...
        if($doc){
            ...
            foreach ($rec as $r){
                    ...
                    if( $jids ){
                        ...
                        if($pars){
                            foreach ($pars->toArray() as $par){
                                ...
                                $prec[] = $nr;
                                $rec_ids[] = $par['email'] ?? new ObjectId($par['_id']);
                            }
                        }
                    }
                }
                ...
                if( ( (is_null($r['obj']['email']) || $r['obj']['email'] == '') ) || ( isset($r['obj']['datatype']) && $r['obj']['datatype'] == 'disposisi' ) ){
                    ...
                    if( $jids ){
                        ...
                        $pars = User::whereIn('employeeId', $jits )->get();
                        if($pars){
                            foreach ($pars->toArray() as $par){
                                $nr = [];
                                $nr['key'] = $par['_id'];
                    ...
                    if( $jids ){
                        ...
                        if($pars){
                            foreach ($pars->toArray() as $par){
                                ...
                                $prec[] = $nr;
                                $cc_ids[] = $par['email'] ?? new ObjectId($par['_id']) ;
                            }
                        }
                    }
                }
            ...
            try {
                ...
                $cc_users = User::whereIn('_id', $cc_ids)
                    ->orWhere(function($q) use ($cc_ids){
                        $q->whereIn('email', $cc_ids);
                    })
                    ->get();
