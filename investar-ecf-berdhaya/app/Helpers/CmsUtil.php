...
class CmsUtil
{
    ...
    public static function singleBlock($content, $blocktemplate, $layout, $aux = [] )
    {
        $template = 'cms.theme.'.$layout.'.'.$blocktemplate;
        ...
        $template = 'cms.theme.default.'.$blocktemplate;
        if(View::exists($template)){
            return view($template)
                ->with('content', $content)
                ->with('aux', $aux)
    ...
    {
        $art = Article::where('slug','=', trim( strtolower( $slug ) ) )->first();
        if($art){
            return $art->toArray();
        }else{
            return [];
    ...
    public static function getArticleByTag($slug)
    {
        $art = Article::where('tags','like', '%'.trim( strtolower( $slug ).'%' ) )->first();
        if($art){
            return $art->toArray();
        }else{
            return [];
    ...
    public static function getArticlesByTag($slug)
    {
        $art = Article::where('tags','like', '%'.trim( strtolower( $slug ).'%' ) )->get();
        if($art){
            return $art->toArray();
        }else{
            return [];
    ...
    public static function getArticleByCategorySlug($slug)
    {
        $art = Article::where('category','=', trim( strtolower( $slug ) ) )->get();
        if($art){
            return $art->toArray();
        }else{
            return [];
    ...
    public static function getArticleBySectionSlug($slug)
    {
        $art = Article::where('section','=', trim( strtolower( $slug ) ) )->get();
        if($art){
            return $art->toArray();
        }else{
            return [];
    ...
    {
        $cat = Category::where('categoryCode','=',trim($slug))->first();
        if($cat){
            return $cat->toArray();
        }else{
            return [];
