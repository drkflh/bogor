            ...
            var uvm = new Vue({
                ...
                methods:{
                    ...
                    doBack(current){
                        @if($is_step_page)
                            @if($backlink != null)
                                window.location = '{{ url($backlink) }}/step/' + ( current - 1 ) + '/'+ this.itemId;            
                            @else
                            window.location = '{{ url($controller_base) }}/step/' + ( current - 1 ) + '/'+ this.itemId + '/' + this.keyword0 + '/' + this.keyword1 + '/' + this.keyword2 ;  
                            @endif
                        @endif
                    },
                    ...
                    postData(act){
                        ...
                        postData.lastStep = this.lastStep;
    ...
    <div id="mainForm" class="h-100" >
        ...
        <b-overlay :show="isLoading" variant="white" opacity="0.85" class="h-100"  >
            ...
            <div class="card h-100 {{ $page_class }} " style="margin: auto;">
                @if($is_step_page)
                    <div class="card-header d-flex flex-row {{  $current_step  == 1 ? 'justify-content-end' : 'justify-content-between' }} ">
                        <button class="btn btn-primary"
                                v-if="{{ $current_step > 1 ? 'true':'false' }}"
                                pill @click="doBack( {{ $current_step }})">
                            <i v-if="!savingInProgress" class="las la-arrow-circle-left"></i>
                            {{ __('Back') }}
                        </button>
                        <button class="btn btn-primary" pill @click="doSave('RESET')">
                            <b-spinner v-if="savingInProgress"  small ></b-spinner > {{  $current_step >= $num_step_page ? __('Finish') : __('Next')}}
                            <i v-if="!savingInProgress" class="las la-arrow-circle-right"></i>
                        </button>
                    </div>
                @else
                            ...
                            <div class="d-inline-flex">
                                ...
                                <button
                                    @if($can_lock)
                                        v-if="!lock"
                                    @endif
                                    @click="doSave" class="btn btn-primary mr-3">
                                    <i v-if="!savingInProgress" class="las la-save"></i>
                                    <b-spinner v-if="savingInProgress"  small ></b-spinner > {{__( $save_button_label )}}
                                </button>
                                @if( $non_closing_save )
                                    <button class="btn btn-primary" pill @click="doSave('RESET')">
                                        <i v-if="!savingInProgress" class="las la-save"></i>
                                        <b-spinner v-if="savingInProgress"  small ></b-spinner > {{__('Save & New')}}
                                    </button>
                                @endif
                            </div>
                        @endif
                ...
                <div class="card-body" >
                    ...
                    <div class="d-md-block d-sm-block d-xs-block">
                        <horizontal-timeline
                            :items="stepProgress"
                        ></horizontal-timeline>
                    </div>
                    @if($mode == 'edit' || $mode == 'add' || $mode == 'revise')
                        @if(env('SKIP_VALIDATION', true ))
                            <?php
                            if(strpos($yml_file,'_controller') === false){
                ...
                @if($is_step_page)
                    <div class="card-footer d-flex flex-row {{  $current_step  == 1 ? 'justify-content-end' : 'justify-content-between' }} ">
                        <button class="btn btn-primary"
                                v-if="{{ $current_step > 1 ? 'true':'false' }}"
                                pill @click="doBack( {{ $current_step }})">
                            <i v-if="!savingInProgress" class="las la-arrow-circle-left"></i>
