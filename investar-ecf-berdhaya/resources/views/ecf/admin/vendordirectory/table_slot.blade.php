    <template v-else-if="props.column.field == 'coyName'">
        <b>Company</b><br>@{{ props.row.coyName ?? '-' }}<br>
        <b>Website</b><br>
        <a v-if="!_.isNull(props.row.website)" :href="normalizeUrl(props.row.website)" target="_blank">
            @{{ props.row.website }}
        </a>
        <template v-else>
            -
        </template><br>
        <b>Vendor Class</b><br>@{{ props.row.vendorClass ?? '-' }}<br>
        <b>Vendor Code</b><br>@{{ props.row.vendorCode ?? '-' }}<br>
    </template>
    ...
    <template v-else-if="props.column.field == 'picContacts'">
        <template v-if="!(_.isEmpty(props.row.picContacts) || props.row.picContacts.length == 0 )">
            <template v-for="contact in props.row.picContacts">
                <i class="las la-xs" :class="contact.contactDefault?'fa-user-check':'fa-user'" :style="contact.contactDefault?'color:green; margin-right:5px;':'color:grey; margin-right:8px;'" style="font-size:8pt;"></i> @{{ contact.contactName }} <br>
                <i class="las la-phone fa-xs" style="font-size:8pt; margin-right:7px;"></i> @{{ contact.contactPhone }} <br>
                <i class="las la-at fa-xs" style="font-size:8pt; margin-right:8px;"></i>
