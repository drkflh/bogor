...
<div id="page-container" class="main-content-boxed">
    ...
    <main id="main-container">
                                    ...
                                    <div class="form-group row">
                                        <div class="col-12">
                                            <select id="clinic" name="clinic"
                                                    class="form-control {{ $errors->has('clinic') ? ' is-invalid' : '' }}"
                                                    required autofocus >
                                                @foreach(\App\Helpers\App\Kmn\KmnUtil::getClinics() as $op)
        ...
        <?php
             \App\Helpers\App\Kmn\KmnUtil::getDocClinics();
        ?>
...
<script>
    $(document).ready(function(){
            ...
            var lookup = {!! json_encode( \App\Helpers\App\Kmn\KmnUtil::getDocClinics() ) !!};

            $('#login').keyup(function(){

                var select = $('#clinic');

                if(_.has(lookup, this.value)){
                    var selector = _.get( lookup, this.value );

                    $('option', select).remove();

                    _.forEach(selector, s=>{
                        var txt = s.cliCode + ' ' + s.cliName;
                        var vl = s.cliCode;
                        var op = new Option( txt , vl );
                        select.append( op );
                    });
                    ...
                    if( _.has(lookup, 'defOpt')){
                        var defOpt = _.get(lookup, 'defOpt');
                        var select = $('#clinic');
                        $('option', select).remove();
