...
<head>
    ...
    <link href="{{ url('themes/nobleui/lib/@fortawesome/fontawesome-free/css/all.min.css') }}" rel="stylesheet">
...
<body onload="spinner()" class="{{ env('SIDEBAR_DEFAULT_OPEN')?'':'sidebar-folded' }} {{ ( env('NOBLE_OPT_THEME','dark') == 'dark' )?'sidebar-dark':'' }}"
      style="background-image: url( '{{ url( env('BG_PATTERNS', 'images/patterns/5086.jpg') ) }}' );background-repeat: repeat;background-size: initial; background-position: bottom;background-attachment: fixed;" >
    <div class="main-wrapper" id="content-block" >
            ...
            <div class="navbar-content" id="nav-app">
                        ...
                        <div class="col-12" id="quickTaskForm">
                            <validation-observer v-slot="{ invalid }" ref="quickTaskForm">
                                <validation-provider rules="required" v-slot="{ errors }" name="title">
                                    ...
                                    <input type="text" name="title" v-model="title" class="form-control" ></input>
                                    <div style="color: rgba(189, 61, 61, 0.839)">@{{ errors[0] }}</div>
                                </validation-provider>
                                <validation-provider rules="required" v-slot="{ errors }" name="description">
                                    <label>Description</label>
                                    <input type="text" name="description" v-model="description" class="form-control" ></input>
                                    <div style="color: rgba(189, 61, 61, 0.839)">@{{ errors[0] }}</div>
                                </validation-provider>
                ...
                @if($with_timetracking)
                    ...
                    <li class="nav-item nav-apps" style="text-align:right;">
                        ...
                        >
                            <i class="lar la-1_5x la-plus-square mr-1"></i> Task
                        </button>
                    </li>
                    <li class="nav-item nav-apps" style="text-align:right;">
                        <button v-show="clockIn" @click="toggleTimer()"
                                ...
                                class="mr-2"
                        >
                            <i :class="timerRunning ? 'lar la-1_5x la-stop-circle':'lar la-1_5x la-play-circle'" class="mr-1" ></i> @{{ timerRunning ? 'Stop':'Start' }}
                        </button>
                    </li>
...
{{--                        </div>--}}
                ...
                @if(env('WITH_MULTI_LANG', false))
                    ...
                    <li class="nav-item dropdown" id="langContainer">
                        <form method="get" action="{!! url()->full() !!}" >
                            <select name="lang" class="form-data custom-select" onchange="this.form.submit()" style="min-width: 100px;">
...
{{--                            </div>--}}
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle mr-1" href="#" id="cartDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
...
{{--                            </div>--}}
                            <div class="dropdown-body" style="max-height:300px;overflow-y: auto;" >
                                <a v-for="c in cart" :href="'{{ url('sms/inventory/etalase/view') }}/' + c._id " class="dropdown-item mt-2 mb-2 p-2 d-flex" style="height: fit-content !important;" >
                                    <div class="icon text-center align-self-start">
                                        <i class="las la-envelope" style="margin: 0px;"></i>
                                    </div>
                                    <div class="content">
                                        <p class="ellipsis" style="width:200px;font-size:9pt;" >@{{ c.product_name_master ?? '' }}</p>
                                        <p class="" style="width:200px;font-size:8pt;" class="sub-text text-muted">
                                            @{{ c.orderQty }} × </span>@{{ c.currency }} @{{ c.price }}
                                        </p>
                                    </div>
                                </a>
                            </div>
                            <div class="dropdown-footer d-flex align-items-end justify-content-center">
                                <div class="shopping-cart-total">
                                    <h4 style="text-align:right;">Total <span>@{{ formatCurrency(totalBill) }}</span></h4>
                                    <div>
                                        <a href="{{ url('sms/inventory/shopping-cart') }}" class="btn btn-outline-warning">View cart</a>
                        ...
                        <div class="dropdown-menu" aria-labelledby="profileDropdown">
                            <div class="dropdown-header d-flex flex-column align-items-center">
                                ...
                                <div class="info text-center">
                                    ...
                                    <p class="email text-muted mb-3">{{ env('REG_EMAIL', false) ? Auth::user()->email : Auth::user()->mobileString }}</p>
                                    <p class="email text-muted mb-3">{{ __('Language') }} {{ $lang }}</p>
                                </div>
                                @endif
                            </div>
        ...
        <div class="page-content h-100" id="app" style="padding: 16px !important;">
            @yield('content')
            @yield('modal')
        </div>
