        ...
        $(document).ready(function () {
            ...
            var tvm = new Vue({
                ...
                methods: {
                    ...
                    async changeSingleApprovalStatus(row){
                        this.selectedRow = row;
                        var aux = {
                            id : row._id,
                        ...
                        if ( !_.isNull(this.approvalShown) && typeof this.approvalShown === 'function') {
                            await this.approvalShown(row);
                        }else{
                            console.log('no function call on approval shown');
                        }
...
@section('content')
    ...
    <div class="h-100 mh-100">
        ...
        @if( $table_component == 'grid-view-2')
            <div class="w-100" style="display:inline-block;width:fit-content;margin-right: 16px;">
            @php $shoppingcart = \App\Models\Ecf\ShoppingCart::where('cartSession', '=', Auth::user()->cartSession)->count(); @endphp
            @if( $shoppingcart > 0)
                <a href="{{ url('check-out') }}" class="btn btn-danger" style="background: #E14D2A; border: none;"><i data-feather="shopping-cart"></i>  Check Out</a>
                <a href="{{ url('ecf/cart/clear') }}" onclick="return confirm('{{ __('Are you sure you want to delete all item in your cart?') }}')" class="btn btn-warning">Clear Shopping Cart</a>
            @endif
            <div><h4>There are @php echo $shoppingcart; @endphp product in your cart</h4></div>
            ...
            @else
                <div class="{{ $grid_card_layout ?? 'card-columns' }} p-1">
                    <div class="col" v-for="item in rows" style="padding: 15px;">
                        <div class="card {{ $grid_card_class ?? '' }}" style="background-color: white;border: none;">
                            @if( View::exists($grid_item_view) )
                                @include($grid_item_view)
                            @else
                                <div class="col-4">
        ...
        @if( $table_component == 'grid-view-3')
            <div class="w-100" style="display:inline-block;width:fit-content;margin-right: 16px;">
            @php $inventoryitem = \App\Models\Ecf\InventoryItem::where('masterId', '=', Auth::user()->_id)->count(); @endphp
            <div><h4>There are @php echo $inventoryitem; @endphp product in your inventory</h4></div>
            ...
            @else
                <div class="{{ $grid_card_layout ?? 'card-columns' }}  row row-cols-1 row-cols-md-2 p-1">
                    <div class="col" v-for="item in rows" style="padding: 15px;">
                        <div class="card {{ $grid_card_class ?? '' }}" style="background-color: white;border: none;">
                            @if( View::exists($grid_item_view) )
                                @include($grid_item_view)
        ...
        @if( $table_component == 'grid-view-4')
            <div class="w-100" style="display:inline-block;width:fit-content;margin-right: 16px;">
            @php $transactionHistory = \App\Models\Ecf\Invoice::where('ownerId', '=', Auth::user()->_id)->count(); @endphp
            @php $transactionHistoryUnpaid = \App\Models\Ecf\Invoice::where('ownerId', '=', Auth::user()->_id)->where('status', '=', 'OPEN')->count(); @endphp
            @php $transactionHistoryPaid = \App\Models\Ecf\Invoice::where('ownerId', '=', Auth::user()->_id)->where('status', '=', 'PAID')->count(); @endphp

            <div><h4>You have @php echo $transactionHistory; @endphp Transaction. <sup class="text-muted">(
            @if($transactionHistoryUnpaid > 0)
               Unpaid: @php echo $transactionHistoryUnpaid; @endphp
            @endif
            @if($transactionHistoryPaid > 0)
                | Paid: @php echo $transactionHistoryPaid; @endphp
            @endif
            )</sup>
            </h4></div>
        ...
        @endif
        @if( $table_component == 'grid-view-5')
            <div class="w-100" style="display:inline-block;width:fit-content;margin-right: 16px;">
                @if( View::exists($table_additional_view) )
                    @include($table_additional_view)
                @endif
        ...
        @if( $table_component != 'grid-view' && $table_component != 'full-calendar' && $table_component != 'grid-view-2' && $table_component != 'grid-view-3' && $table_component != 'grid-view-4' && $table_component != 'grid-view-5')
                            ...
                            <div style="margin: 18px 0px;position: absolute;right: 0px;">
                                ...
                                @if( $can_approve )
                                    <button-single-approval-pin-ajax
                                        ...
                                        signaturedefaulturl="{{ url( env('DEFAULT_CARD_IMAGE')) }}"
