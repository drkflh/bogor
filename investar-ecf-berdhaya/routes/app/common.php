...
if( env('WITH_FRONTEND', false )){
    Route::get('/', 'Open\HomeController@getView');
    Route::get('/home', 'Open\HomeController@getView');
    Route::get('/home/{id?}', 'Open\HomeController@getView');
}

Route::get('/page/{slug?}', 'Open\HomeController@getPage');

Route::get('email/verify', 'Auth\RegisterController@showRegistrationVerify');
Route::get('/verify', 'Auth\RegisterController@showRegistrationVerify');
Route::post('/verify/resend', 'Auth\RegisterController@postVerifyResend');

// dipanggil setelah register
Route::get('/otp/{cartSession}', 'Auth\RegisterController@getOtpSelect')->name('otpSelect');
Route::get('/otp/verify/{cartSession}', 'Auth\RegisterController@getOtpResend')->name('otpVerify');
// dipanggil setelah memilih antara email atau mobile
Route::post('/otp/verify/', 'Auth\RegisterController@postOtpResend')->name('otpResend');
Route::post('/otp/verify/success', 'Auth\RegisterController@postOtpAfterResend')->name('afterResend');
...
\App\Helpers\Util::makeRoute('queue-failed-log', 'Core\QueueFailedLogController');
...
Route::get('/pass/forgot', 'Auth\ForgotPasswordController@getForgot');
Route::post('/pass/forgot', 'Auth\ForgotPasswordController@postForgot')->name('postForgotPass');
Route::get('/pass/reset/{cartSession?}', 'Auth\ForgotPasswordController@getNewPassword');
Route::post('/pass/reset/{cartSession?}', 'Auth\ForgotPasswordController@postNewPassword')->name('postNewPass');


Route::get('/pass/reset/{session}', 'Auth\ResetPasswordController@getReset');
Route::post('/pass/reset/{session}', 'Auth\ResetPasswordController@postReset');
...
Route::get('web-console', function(){
...
//profile completer
Route::get('personal-profile/{id?}', 'Core\UserController@editPersonalProfile');
Route::get('biz-profile/{id?}', 'Core\UserController@editBizProfile');
Route::get('second-profile/{id?}', 'Core\UserController@editSecondProfile');
...
Route::group([ 'prefix'=>'workflow', 'middlewareGroup'=>['web'] ], function(){
    ...
    Route::get('/time/report/detail', 'Workflow\Time\TimeReportController@getDetail');
    Route::post('/time/report/get-seq', 'Workflow\Time\TimeReportController@postGetSeq');
...
Route::group([ 'prefix'=>'obj', 'middlewareGroup'=>['web'] ], function(){
    ...
    \App\Helpers\Util::makeRoute('article-template', 'Obj\ArticleTemplateController');
...
Route::group([ 'prefix'=>'mms', 'middlewareGroup'=>['web'] ], function(){
    ...
    Route::post('/qontak/add-tpl', 'Mms\NotificationTemplateController@postQontakTpl');
    Route::post('/qontak/check-tpl', 'Mms\NotificationTemplateController@getQontakTpl');

    Route::post('/message-gateway/wa-session', 'Mms\MessageGatewayController@getWaSession');
    Route::post('/message-queue/exec-broadcast', 'Mms\MessageQueueController@postExecuteQueue');
...
Route::group([ 'prefix'=>'dms', 'middlewareGroup'=>['web'] ], function(){
    ...
    Route::post('/embed-qr', 'Dms\RepositoryController@postEmbedQr');
...
//        $pdf->Write(0, $data);
    ...
    Route::get('image', function(\Illuminate\Http\Request $request){
        ...
        $url = $request->get('u');
        $bucket = $request->get('b');
        $w = $request->w ?? 320;
        $h = $request->h ?? 240;

        try {
            $img = Image::make($url);//manipulate image
            $img->resize($request->w, $request->h, function ($constraint) {
                $constraint->aspectRatio();
            });// create response and add encoded image data
            $response = Illuminate\Support\Facades\Response::make($img->encode('jpg'));// set content-type
            $response->header('Content-Type', 'image/jpg');// output
            return $response;
        } catch (Exception $e) {
        }
...
\App\Helpers\Util::makeRoute('workflow/time/report', 'Workflow\Time\TimeReportController');
...
\App\Helpers\Util::makeRoute('reference/select-options', 'Reference\SelectOptionsController');
\App\Helpers\Util::makeRoute('mms/notification-template', 'Mms\NotificationTemplateController');
\App\Helpers\Util::makeRoute('reflow/cycle', 'Reflow\CycleController');
\App\Helpers\Util::makeRoute('reflow/decision-rule', 'Reflow\DecisionRuleController');
\App\Helpers\Util::makeRoute('core/console-log', 'Core\ConsoleLogController');
\App\Helpers\Util::makeRoute('mms/qontak/send-log', 'Qontak\SendLogController');
\App\Helpers\Util::makeRoute('mms/qontak/broadcast-status-log', 'Qontak\BroadcastStatusLogController');
\App\Helpers\Util::makeRoute('mms/qontak/wa-template', 'Qontak\WaTemplateController');
\App\Helpers\Util::makeRoute('mms/qontak/channel', 'Qontak\ChannelController');
\App\Helpers\Util::makeRoute('mms/qontak/category', 'Qontak\CategoryController');
\App\Helpers\Util::makeRoute('mms/qontak/file-upload', 'Qontak\FileUploadController');
\App\Helpers\Util::makeRoute('mms/qontak/lang', 'Qontak\LangController');
\App\Helpers\Util::makeRoute('mms/wa-message-log', 'Mms\WaMessageLogController');
Route::post('ajax/nw', 'AjaxController@postNumber2Word');
